...
require ( dirname( __FILE__ ) . '/_inc/bp-follow-widgets-display.php' );
...
function bp_follow_setup_nav() {
	...
	$memberType = xprofile_get_field_data('Member Type', bp_displayed_user_id());
	if ( $memberType == "Fans" ):
	bp_core_new_nav_item( array(
		'name'                => sprintf( __( 'Fan Of <span>%d</span>', 'bp-follow' ), $counts['following'] ),
	...
	if ( $memberType == "Acts" ):
	bp_core_new_nav_item( array(
		'name'                => sprintf( __( 'Fans <span>%d</span>', 'bp-follow' ), $counts['followers'] ),
...
function bp_follow_action_start() {
	...
	if ( bp_follow_is_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) )
		bp_core_add_message( sprintf( __( 'You are already a fan of %s.', 'bp-follow' ), bp_get_displayed_user_fullname() ), 'error' );
	else {
		if ( !bp_follow_start_following( array( 'leader_id' => bp_displayed_user_id(), 'follower_id' => bp_loggedin_user_id() ) ) )
			...
			bp_core_add_message( sprintf( __( 'You are now a fan of %s.', 'bp-follow' ), bp_get_displayed_user_fullname() ) );
	}
...
function bp_follow_ajax_action_start() {
	...
	if ( bp_follow_is_following( array( 'leader_id' => $_POST['uid'], 'follower_id' => bp_loggedin_user_id() ) ) )
		$message = __( 'Already a fan', 'bp-follow' );
	else {
		if ( !bp_follow_start_following( array( 'leader_id' => $_POST['uid'], 'follower_id' => bp_loggedin_user_id() ) ) )
			...
			$message = __( 'You are now a fan', 'bp-follow' );
	}
